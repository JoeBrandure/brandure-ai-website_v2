...
class AnimationController {
    ...
    // Constants
    private readonly changeEventTime = 0.15 // Reduced to show more of the final effect earlier
    private readonly cameraZ = -400
    private readonly cameraTravelDistance = 4000
    private readonly startDotYOffset = 0 // Removed vertical offset to center perfectly
    private readonly viewZoom = 100
    private readonly numberOfStars = 5000
    private readonly trailLength = 120 // Increased for longer trail
    ...
    private setupTimeline() {
        this.timeline
            .to(this, {
                time: 1,
                duration: 45, // Increased from 30s to 45s for smoother, more complete animation
                repeat: -1,
                ease: "none",
    ...
    public render() {
        ...
        // Center the animation exactly in the viewport center
    ...
    render(p: number, controller: AnimationController) {
        ...
        if (q > 0) {
            ...
            if (displacementProgress < 0.3) {
                ...
                easing = controller.lerp(displacementProgress, Math.pow(displacementProgress, 2), t);
            } else if (displacementProgress < 0.7) {
                const t = (displacementProgress - 0.3) / 0.4;
                easing = controller.lerp(Math.pow(displacementProgress, 2), controller.easeOutElastic(displacementProgress), t);
            } else {
...
export default function LoadingAnimation({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    // Use the smaller dimension as base size to ensure animation fits
    const baseSize = Math.min(dimensions.width, dimensions.height);
    animationRef.current = new AnimationController(canvas, ctx, dpr, baseSize);
  ...
  // Logo click handler for loading transition
  ...
  return (
    ...
    >
             {/* Canvas for spiral animation - centered with flexbox */}
       <canvas
         ref={canvasRef}
         style={{
           width: '100%',
